...
const Wallet = require('../wallets/Wallet').Wallet;
const { generateToken, generateRefreshToken, verifyRefreshToken } = require('../../shared/utils/generateToken');
const { sendOTPEmail, sendWelcomeEmail, sendPasswordResetEmail } = require('../../shared/utils/sendEmail');
...
const login = async (req, res, next) => {
  try {
    ...
    if (user.role === 'subadmin' && user.roleId) {
      const Role = require('../subadmins/Role');
...
const socialLogin = async (req, res, next) => {
  try {
    ...
    if (!user) {
      ...
      user.generateReferralCode();
      await user.save();
      await require('../wallets/Wallet').Wallet.create({ user: user._id });
    }
