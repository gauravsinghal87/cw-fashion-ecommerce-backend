...
const APIFeatures = require('../../shared/utils/pagination');
...
const createProduct = async (req, res, next) => {
  try {
    ...
    const allowed = ['title', 'description', 'category', 'brand', 'images', 'variants', 'tags', 'attributes', 'isReturnable', 'returnWindow', 'metaTitle', 'metaDescription'];
    const productData = {};
...
const getFlashSaleProducts = async (req, res, next) => {
  try {
    ...
    }).populate('vendor', 'storeName').sort('-createdAt').skip((page - 1) * limit).limit(limit).lean();
...
const getNewArrivals = async (req, res, next) => {
  try {
    const products = await Product.find({ isActive: true, status: 'approved' })
      .populate('vendor', 'storeName').limit(12).sort('-createdAt').lean();
    res.json({ success: true, products });
  } catch (error) {
    next(error);
  }
...
const getBestSellers = async (req, res, next) => {
  try {
    const products = await Product.find({ isActive: true, status: 'approved' })
      .populate('vendor', 'storeName').limit(12).sort('-totalSold').lean();
    res.json({ success: true, products });
  } catch (error) {
    next(error);
  }
...
const getTrendingProducts = async (req, res, next) => {
  try {
    const products = await Product.find({ isActive: true, status: 'approved' })
      .populate('vendor', 'storeName').limit(12).sort('-trendingScore').lean();
    res.json({ success: true, products });
  } catch (error) {
    next(error);
  }
...
const getRelatedProducts = async (req, res, next) => {
  try {
    ...
    const products = await Product.find({
      ...
      isActive: true,
      status: 'approved'
    }).limit(8).populate('vendor', 'storeName').lean();
    res.json({ success: true, products });
  } catch (error) {
    next(error);
  }
...
const getProductsByVendor = async (req, res, next) => {
  try {
    ...
    const products = await features.query.populate('vendor', 'storeName storeSlug storeLogo storeDescription storeRating totalRatings').lean();
...
const getProductsByCategory = async (req, res, next) => {
  try {
    ...
    const products = await features.query.populate('vendor', 'storeName').lean();
...
const getReviews = async (req, res, next) => {
  try {
    const product = await Product.findById(req.params.id).select('rating numRatings numReviews').lean();
    if (!product) return res.status(404).json({ success: false, message: 'Product not found' });
