...
const AuditLog = require('../auditLogs/AuditLog');
const Notification = require('../notifications/Notification');
const { sendVendorApprovalEmail, sendOTPEmail, sendWelcomeEmail } = require('../../shared/utils/sendEmail');
const slugify = require('slugify');
const crypto = require('crypto');
const { generateToken, generateRefreshToken } = require('../../shared/utils/generateToken');
...
const updateVendorProfile = async (req, res, next) => {
  try {
    const allowed = ['storeName', 'storeDescription', 'storeLogo', 'storeBanner', 'bankAccount', 'aadhaarNumber', 'panNumber', 'returnPolicy', 'settings', 'warehouseAddress'];
    const updates = {};
    allowed.forEach(f => { if (req.body[f] !== undefined) updates[f] = req.body[f]; });
    const vendor = await Vendor.findOneAndUpdate(
      { user: req.user._id },
      { $set: updates },
      { new: true, runValidators: true, select: '+aadhaarNumber' }
    );
    res.json({ success: true, vendor });
  } catch (error) {
    next(error);
...
const getVendorDashboard = async (req, res, next) => {
  try {
    ...
    const [
      ...
      require('../returns/Return').countDocuments({ vendor: vendor._id, status: { $in: ['return_requested', 'return_approved', 'pickup_scheduled'] } }),
      require('../wallets/Wallet').Wallet.findOne({ user: req.user._id }),
      Order.aggregate([
        { $unwind: '$items' },
...
const uploadBulkProducts = async (req, res, next) => {
  try {
    ...
    const created = [];
    const allowed = ['title', 'description', 'category', 'brand', 'images', 'variants', 'tags', 'attributes', 'isReturnable', 'returnWindow'];
...
const createAuditLog = async (vendorId, userId, action, resource, details) => {
  await AuditLog.create({
    user: userId, resourceId: vendorId, action, resource, description: details, ip: '127.0.0.1'
  });
};
...
const generateInvoice = async (req, res, next) => {
  try {
    ...
    const invoice = {
      ...
      vendorAddress: `${vendor.warehouseAddress?.addressLine1 || ''}, ${vendor.warehouseAddress?.city || ''}, ${vendor.warehouseAddress?.state || ''} - ${vendor.warehouseAddress?.pincode || ''}`,
