...
const { generateOrderNumber, generateInvoiceNumber } = require('../utils/orderNumberGenerator');
...
const createOrder = async (req, res, next) => {
  try {
    ...
    for (const item of items) {
      ...
      const now = new Date();
      const commission = await require('../models/Commission').findOne({
        $or: [
...
const cancelOrder = async (req, res, next) => {
  try {
    ...
    for (const item of order.items) {
      ...
      try { await require('../models/Settlement').findOneAndUpdate({ orderItem: item._id, status: 'pending' }, { status: 'cancelled' }); } catch (e) { /* non-blocking */ }
    }
